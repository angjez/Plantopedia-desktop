...
def choice():
    while True:
        ListOfPlants.print_list(ListOfPlants.list_of_plants)
        answer = input("Would you like to add a new plant or delete some? (add/delete/exit): ")
        if answer == "add":
            ListOfPlants.add_new_plant(ListOfPlants.list_of_plants)
        elif answer == "delete":
            if not ListOfPlants.list_of_plants:
                print("You have no plants saved (nothing to delete).")
                time.sleep(1)
            else:
                delete_plant(ListOfPlants.list_of_plants)
        elif answer == "exit":
...
def delete_plant(list_of_plants):
    while True:
        if not list_of_plants:
            ...
            if answer != "exit":
                result = ListOfPlants.delete_from_list(ListOfPlants.list_of_plants, answer)
                if result:
